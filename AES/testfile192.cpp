...
int main(int argc, char **argv) {
	...
	word out[4];
	clock_t start, end;
	rand_word_array(state, 192);
    rand_word_array(key, 192);
    // printf("plaintext: ");
    // print_verilog_hex(state, 192);
    // printf("\n");
    // printf("key:       ");
    // print_verilog_hex(key, 192);
	// printf("\n");
	...
	//C Implementation 
	start = clock();
	encrypt_192_key_expand_inline_no_branch(out, key);
	encrypt_192_key_expand_inline_no_branch(out, key);
	encrypt_192_key_expand_inline_no_branch(out, key);
	encrypt_192_key_expand_inline_no_branch(out, key);
	encrypt_192_key_expand_inline_no_branch(out, key);
	...
	Vaes_192 *aes_192 = new Vaes_192;

	aes_192->state = {REV(state[3]), REV(state[2]), REV(state[1]), REV(state[0])};
	aes_192->key = {REV(key[3]), REV(key[2]), REV(key[1]), REV(key[0])};
	out[3] = REV(out[3]);
	out[2] = REV(out[2]);
	out[1] = REV(out[1]);
	out[0] = REV(out[0]);
	...
	//std::cout << aes_192->out << std::endl;
	// if(out[3] != aes_192->out){
	// 	std::cout << "E"<< std::endl;
	// }
